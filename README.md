...
- **只读监控**：风扇转速（CPU / GPU1）、CPU/GPU 温度（摄氏度）、原始占空比、风扇曲线。
- **写入控制**：风扇模式（`auto` / `quiet` / `max` / `maxq` / `custom`）、
  **自定义四段风扇曲线**（命令 14，写入后自动切到 `custom`）与性能模式
...
- **可离线测试**：无需硬件，全部用手写 fixture 测试（149 Rust + 25 UI 后端 + 119 前端）。
